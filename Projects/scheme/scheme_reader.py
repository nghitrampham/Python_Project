    ...
    try:
        if src.current() is None:
            ...
            if  read_tail(src) != nil:
                raise SyntaxError('Exactly 1 element after the dot')
            return current
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
            #this will remove the first element of src, and apply read_scheme on it
            first_of_src = scheme_read(src)
            # recursively call read_tail
            rest_src = read_tail(src)
            #return in Pair form
            return Pair(first_of_src, rest_src)
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
