...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    # evaluates call expressions
        ...
        if not check_procedure(operator):
...
class Frame:
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 3
        # binds the value to that symbol in the frame
    ...
    def lookup(self, symbol):
        ...
        # lookup that symbol in the parent if the parent exists.
        elif self.parent is not None:
            return Frame.lookup(self.parent, symbol)
        # else raise an SchemeError
        else:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        # check if the number of argument values = number of formal parameters
        if len(formals) != len(vals):
            raise SchemeError('#argument does not match with #parameters')
        else:
            # Binds formal parameters to their correspondig argument
            # values in the newly created frame
            while len(formals) != 0:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        while args is not nil:
            ...
            args = args.second
        # BEGIN PROBLEM 4
        # if self.use_env is True, then add the current environment env
        if self.use_env == True:
            python_args.append(env)
        try:
            # Call self.fn on all of those arguments
            return self.fn(*python_args)
            # raise error if wrong number of parameters were passed
        except TypeError:
            raise SchemeError('Bad input')
        # END PROBLEM 4
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        if isinstance(self, LambdaProcedure):
            # create new frame and do bingings
            new_env = env.make_child_frame(self.formals, args)
            # new frame isa child of the frame in which the lambda is defined
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        # add to the current enviroment and return the symbol
        env.define(target, scheme_eval(expressions.second.first, env))
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        # get the name which is targer.first
        # then get the parameter/formals (target.second) and
        parameter = target.second
        # body which is expressions.second
        body = expressions.second
        # make expression for do_lambda_form
        make_lambda = Pair(parameter, body)
        # add to enviroment
        env.define(target.first, do_lambda_form(make_lambda,env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    # returns its operand expression without evaluating it
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    # get the body of lambda function
    body = expressions.second
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    # return true if If there are no sub-expressions
    if len(expressions) == 0:
        return True
    # evaluate until the first false or the last evaluation without false
    while len(expressions) != 0:
        # binds the value
        value = scheme_eval(expressions.first,env, expressions.second == nil)
        # return first false value if found
...
def do_or_form(expressions, env):
    ...
    while len(expressions) != 0:
        value = scheme_eval(expressions.first, env, expressions.second == nil)
        # return value if first true value is found
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            # true predicate does not have a corresponding
            # result sub-expression, return the predicate value
            if clause.second == nil:
                return test
            else:
                #  if multiple expressions, evaluate them all
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    # initializng parameters and values for Pair
    parameters = nil
    values = nil
    while len(bindings) != 0:
        if len(bindings.first) >2 or len(bindings.first) <2:
            raise SchemeError('too many values in binding')
        else:
            # check the structure of each binding
            check_form(bindings.first, 2,2)
            # forming Pair of formal parameters (symbols) and a Pair of values,
    ...
    # returns a new frame with all the symbols bound to the corresponding values.
...
class MuProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a dinamically-scoped call evaluated in environment ENV."""
        # BEGIN PROBLEM 12
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # evaluates the operands
            arguments = rest.map(lambda operand: scheme_optimized_eval(operand,env))
