...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        if self.exit is not None: # If the Place has an exit,
            self.exit.entrance = self # then the exit's entrance is set to that Place.
...
class Insect(object):
    ...
    is_ant = False
    damage = 0
    # add a new attribute to the Insect class named watersafe
    watersafe = False
...
class Bee(Insect):
    ...
    name = 'Bee'
    damage = 1
    # since bee can fly, we set their watersafe property == True
    watersafe = True
...
class Ant(Insect):
    ...
    def can_contain(self, other_ant):
    	if self.container is True and other_ant.container is False and self.ant is None:
    		return True
    	return False
...
class HarvesterAnt(Ant):
    ...
    name = 'Harvester'
    implemented = True
    # change food_cost for Harvester Ant
    food_cost = 2
...
class ThrowerAnt(Ant):
    ...
    implemented = True
    damage = 1
    # change food_cost for Thrower Ant
    food_cost = 3
    min_range = 0
    max_range = 10
    ...
    def nearest_bee(self, hive):
        ...
        #make sure the max_range is correct in for loop
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # use of inheritance to reuse the functionality of the Place class to Water
        Place.add_insect(self, insect)
        # reduce insect's armor to 0
...
class FireAnt(Ant):
    ...
    name = 'Fire'
    # initialize damage and food_cost of FireAnt
    damage = 3
    food_cost = 5
...
class LongThrower(ThrowerAnt):
    ...
    name = 'Long'
    # initialize food_Cost and min_range for LongThrower
    food_cost = 2
    min_range = 5
    implemented = True
    #
    # def neart_bee(self, hive):
    #     count  = 0
    #     current_place = self.place
    #     bee = None
    #     while current_place != hive and bee == None:
...
class ShortThrower(ThrowerAnt):
    ...
    # def nearest_bee(self, hive):
    # 	count, bee, current_place = 0, None, self.place
    # 	while current_place != hive and bee == None:
    #         # set condition to at most 3 entrance transitions to return a random bee
    # 		if  count <= self.max_range:
    # 			bee = random_or_none(current_place.bees)
    # 		current_place = current_place.entrance
    #         # keep traack of entrance transitions
    # 		count +=1
    # 	return bee
...
class WallAnt(Ant):
    ...
    """Wall Ant does nothing each turn. It's helpful because
    it has a large amount of armor
    """
    name = "Wall"
    implemented = True
    food_cost = 4
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        #Go through a list of all bees at the current place and reduce the armor of each bee
    	for bee in self.place.bees[:]:
    		bee.reduce_armor(self.damage)

class ScubaThrower(ThrowerAnt):
    """Scube Thrower is a watersafe, and does not lose its armor
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        #Reduce the bee armor
    	bee.reduce_armor(bee.armor)
        #After eating a bee, reset the digesting attribute so it can't eat again after 3 times
    	self.digesting = self.time_to_digest


    def action(self, colony):
        if self.digesting == 0:
            #Eat a random bee in place when the ant hasn't eaten any bee yet
            bee = random_or_none(self.place.bees)
            if bee is not None:
                self.eat_bee(bee)
        else:
            #When the ant already ate a bee, reduce digesting by 1 each time action is called
            self.digesting -=1
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        #If the ant is an Harvester Ant, produces 1 extra food by calling action
    	if type(self.ant) is HarvesterAnt:
    		self.ant.action(colony)
        #Reduce the armor of all bees in current place
    	for bee in self.place.bees[:]:
            if self.ant is None:
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
     ...
     #    #Initalize Queen Ant for the first time calling QueenAnt
    	# if QueenAnt.true_queen:
    	# 	ScubaThrower.__init__(self)
    	# 	self.true_queen = True
    	# 	QueenAnt.true_queen = False
     #    #For the second time, change true_queen attribute to false
     #    #to show that this is not a true queen
    	# else:
    	# 	ScubaThrower.__init__(self)
    	# 	self.true_queen = False
    ...
    def action(self, colony):
        ...
        """
        #If not true queen, reduce armor to 0 upon taking the first action
        if self.true_queen is False:
            ...
            while current_place is not None:
                ...
                if ants is not None:
                    if ants.container and ants.ant and ants.ant not in self.ant_list:
                        ants.ant.damage *= 2
                        self.ant_list += [ants.ant]
                    if ants not in self.ant_list:
                        ants.damage *= 2
                        self.ant_list += [ants]
    ...
    def reduce_armor(self, amount):
        ...
        if self.true_queen and self.armor <= 0:
            bees_win() #signal the game is over when the true queen has no armor left
...
def make_slow(action):
    ...
    def make_action(colony):
        # performs the original action on turns where colony.time is even
        if colony.time %2 == 0:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    alive_bee = [duration]
    # current bee's action
    current_action = bee.action

    def effect_on_bee(colony):
        nonlocal alive_bee
        # if bee lasts for duration turns, arranges to have the new
        # method become the bee's action method for the next duration
        if alive_bee[0]:
            alive_bee[0] -= 1
            # apply effect
            take_action  = effect(current_action)
            return take_action(colony)
        else:
            return current_action(colony)
    bee.action = effect_on_bee
